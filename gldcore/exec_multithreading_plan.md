...
- If you need to change the thread count for multithreading, update the value of `altThreadcount` in the code.
...
This plan provides actionable steps for improving multithreading efficiency in [`gldcore/exec.cpp`](gldcore/exec.cpp), ensuring deterministic, compatible, and portable optimization for typical desktop hardware using `std::thread`. Benchmarking and validation should use [`test_parallel/bench_500_houses.json`](test_parallel/bench_500_houses.json) as a representative workload. The singlethreaded code should remain unchanged for direct comparison. Any existing multithreading code may be changed or removed to maximize performance. Questions can be asked throughout the process if clarification or additional information is required. Special attention should be given to syncing objects with the same parent in the same thread or using parent-level locks to prevent conflicts. All test runs must be error-free, including convergence and other failures. Thread count for multithreading is controlled by `altThreadcount`. Ignore all code and configuration related to `global_threadcount` for this effort. Multithreaded performance must exceed singlethreaded by at least 20%. Ignore the "Parallelism" value in profile output. Update instructions as needed after each iteration. Iterate until multithreading is faster than singlethreading.